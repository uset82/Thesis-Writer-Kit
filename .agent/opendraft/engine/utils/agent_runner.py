...
from utils.openai_adapter import OpenAIChatAdapter
...
def setup_model(model_override: Optional[str] = None) -> Any:
    """
    Initialize and return configured LLM model based on provider.

    Args:
        model_override: Optional model name to override config default

    Returns:
        Model client with a generate_content method
    ...
    if provider == 'openai':
        ...
        return OpenAIChatAdapter(client=client, model=model_name, temperature=config.model.temperature)

    if provider == 'claude':
        raise ValueError("Claude provider is not wired yet. Use gemini or openai.")
...
def research_citations_via_api(
    ...
    config = get_config()
    enable_gemini_grounded = config.model.provider == 'gemini'
    ...
    if not enable_gemini_grounded and verbose:
        safe_print("   Gemini grounded disabled (provider != gemini)")
